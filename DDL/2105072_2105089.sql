...
ADDRESS NUMBER,  
URL VARCHAR2(200),
TYPE VARCHAR2(20) CHECK (UPPER(TYPE) IN ('BRANCH','SUPPLIER','TRANSPORT','CENTRAL','CLIENT')),   --generalization for all these entities
CONSTRAINT LOC_FK FOREIGN KEY(ADDRESS) REFERENCES LOCATIONS(LOCATION_ID) ON DELETE  SET NULL
);       
...
CREATE TABLE CONTACTS (
    ...
    CONSTRAINT CONTACT_OWNER_FK FOREIGN KEY (OWNER_ID) REFERENCES ORGANIZATIONS(ORGANIZATION_ID) ON DELETE CASCADE
);
...
CREATE TABLE FEATURE_NAMES (
    FEATURE_ID NUMBER CONSTRAINT FEATURE_PK PRIMARY KEY,
    FEATURE_NAME VARCHAR2(30) UNIQUE NOT NULL
);
...
TYPE VARCHAR2(30) CHECK(UPPER(TYPE) IN ('CLIENT_ORDER', 'PURCHASE_ORDER','SUPPLY_REQUEST','CONFIRMED_SUPPLY')),    
...
REQUEST_ID NUMBER, 
DEADLINE DATE,
CONSTRAINT SR_FK FOREIGN KEY(REQUEST_ID) REFERENCES PRODUCT_TRANSACTIONS(TRANSACTION_ID) ON DELETE CASCADE,
...
REQUEST_ID NUMBER,
CONSTRAINT CR_REQUEST_FK FOREIGN KEY(REQUEST_ID) REFERENCES SUPPLY_REQUESTS(REQUEST_ID) ON DELETE SET NULL,
...
ORDER_ID NUMBER,
PICKUP_DATE DATE, 
CONSTRAINT PO_FK FOREIGN KEY(ORDER_ID) REFERENCES PRODUCT_TRANSACTIONS(TRANSACTION_ID) ON DELETE CASCADE,
CONSTRAINT PURCHASE_PK PRIMARY KEY(ORDER_ID)
);

CREATE TABLE PRODUCT_TRANSACTION_ITEMS(     --products and their quantities used in each transaction
TRANSACTION_ID NUMBER,
PRODUCT_ID NUMBER,
CONSTRAINT PTI_PK PRIMARY KEY(TRANSACTION_ID,PRODUCT_ID),
QUANTITY NUMBER CHECK(QUANTITY>0),
CONSTRAINT PTI_TRANSACTION_FK FOREIGN KEY(TRANSACTION_ID) REFERENCES PRODUCT_TRANSACTIONS(TRANSACTION_ID) ON DELETE CASCADE,
CONSTRAINT PTI_PRODUCT_FK FOREIGN KEY(PRODUCT_ID) REFERENCES PRODUCTS(PRODUCT_ID) ON DELETE CASCADE
);
...
DESTINATION NUMBER,
CONSTRAINT TRANSPORT_COMPANY_FK  FOREIGN KEY(TRANSPORT_COMPANY_ID) REFERENCES ORGANIZATIONS(ORGANIZATION_ID) ON DELETE SET NULL,
